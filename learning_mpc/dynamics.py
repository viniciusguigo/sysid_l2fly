#!/usr/bin/env python
""" dynamics.py:
Learns a plant dynamics using deep learning.
...
class NNDynamicsModel():
    def __init__(self, 
        ...
        self.input_state = tf.placeholder(
          shape=(None, ob_dim), dtype=tf.float32)
        
        self.input_act = tf.placeholder(
          shape=(None, ac_dim), dtype=tf.float32)
        
        self.target_delta = tf.placeholder(
          shape=(None, ob_dim), dtype=tf.float32)

        self.dyn = build_mlp(tf.concat(
          [self.input_state, self.input_act], axis=1), 
                output_size=ob_dim, 
        ...
        self.loss = tf.losses.mean_squared_error(
          labels=self.target_delta, predictions=self.dyn) 
        
        self.update = tf.train.AdamOptimizer(
          learning_rate).minimize(loss=self.loss)
    ...
    def fit(self, data):
        """
        Takes in a dataset of (unnormalized)states, (unnormalized)actions,
        (unnormalized)next_states and fit the dynamics model going from
        normalized states, normalized actions to normalized state differences
        (s_t+1 - s_t).
        """
        obs = normalize(
          data["observations"], self.normalization["observations"])
        acts = normalize(data["actions"], self.normalization["actions"])
        deltas = normalize(data["next_observations"] - data["observations"],
          self.normalization["deltas"])
        chunk_size = len(data["observations"])
        
        batch_indexes = batch_index(self.batch_size, chunk_size)
        loss = None
        for epoch in range(self.iterations):
          if epoch % 20 == 0: print("Epoch {}/{}: Loss {}".format(
            epoch, self.iterations, loss))
