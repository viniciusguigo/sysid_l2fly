...
A = np.genfromtxt(
'/tmp/expt_1468984536/log.txt',delimiter='\t',dtype=None, names=True)
A['EpRewMean']
...
def configure_output_dir(d=None):
    ...
    if osp.exists(G.output_dir):
        print("Log dir %s already exists! \
...
def log_tabular(key, val):
    ...
    if G.first_row:
        ...
        assert key in G.log_headers, "Trying to introduce a new key %s that \
